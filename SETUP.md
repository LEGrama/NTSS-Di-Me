...
- **가장 흔한 원인: Sheet 이름 불일치**
  ...
  - 코드의 범위 설정 (`Sheet1!A:D`)과 실제 시트 이름이 일치하는지 확인
...
- 해결 방법:
  ...
  2. 코드에서 `range: 'Sheet1!A:D'`를 `range: '시트1!A:D'`로 변경

## 상태 관리 기능

D열의 "상태" 컬럼을 통해 대기명단을 관리할 수 있습니다:

- **미완료**: 신규 등록 시 기본값, 대기 중인 상태
- **진행중**: 작업이 진행 중인 상태 (대기번호에 포함)
- **완료**: 작업이 완료된 상태 (대기번호에서 제외)

### 상태 변경 방법
Google Sheets에서 직접 D열의 값을 수정하여 상태를 관리하세요.

### 대기번호 계산 방식
- "완료" 상태인 사람은 대기번호 계산에서 제외됩니다
- "미완료"와 "진행중" 상태인 사람만 대기번호가 부여됩니다
- 예시:
  - 1번째 등록자: 미완료 → 대기번호 1번
  - 2번째 등록자: 완료 → 제외
  - 3번째 등록자: 미완료 → 대기번호 2번
